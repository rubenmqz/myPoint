 ...
 *
 * Modified by Ruben Mqz in order to be able to show weekdays if date is bettwen 1 and 7 days ago.
 *
 */
  ...
  $.extend($.timeago, {
    ...
    inWords: function(distanceMillis, date) {
      ...
      function substitute(stringOrFunction, number) {
        ...
        return prefix + ' ' + string.replace(/%d/i, value);
      }
      ...
      function weekDay(number) {
        ...
        if (number == 3) return "miércoles";
        if (number == 4) return "jueves";
        if (number == 5) return "viernes";
        if (number == 6) return "sábado";
      }
      ...
      return $.trim([words, suffix].join(separator));
    },
